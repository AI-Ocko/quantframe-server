...
- **Restore:** First check the file you are about to restore: `python3 -c "import sqlite3,sys; print(sqlite3.connect(sys.argv[1]).execute('PRAGMA integrity_check').fetchone()[0])" backups/quantframe-<date>.sqlite` must print `ok`. Then `docker compose stop`, then copy the chosen file over the live database and drop the stale WAL:
  ```bash

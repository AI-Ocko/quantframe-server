...
pub fn blend(inferred: Vec<ItemStats>, closed: Vec<ClosedStats>, mode: PriceSourceMode, guard_pct: i64, profit_basis: ProfitBasis, now: DateTime<Utc>) -> Vec<Effective> {
    ...
    let mut out: Vec<Effective> = inferred
        ...
        .map(|i| match closed_by_key.remove(&(i.item_id.clone(), i.sub_type.clone())) {
            Some(c) => merge(Some(i), c, guard_pct, profit_basis, now),
            None => plain(i),
        })
        .collect();
    out.extend(closed_by_key.into_values().map(|c| merge(None, c, guard_pct, profit_basis, now)));
    out
}
...
mod tests {
    ...
    fn closed(id: &str, volume: f64, moving_avg: f64) -> ClosedStats {
        ClosedStats { item_id: id.into(), sub_type: String::new(), volume, moving_avg: Some(moving_avg), median: Some(moving_avg + 1.0), avg_price: Some(moving_avg + 2.0), min_price: Some(40), max_price: Some(90), week_price_shift: Some(-3.0), range_profit: Some(30.0), days: 7, trades: (volume * 7.0) as i64, warm: true }
    }

    #[test]
    fn inferred_mode_is_the_identity() {
        let rows = vec![inferred("a", 9.0, 70.0, 50.0, 12.0)];
        let out = blend(rows.clone(), vec![closed("a", 30.0, 66.0)], PriceSourceMode::Inferred, 10, ProfitBasis::Spread, now());
        assert_eq!(out.len(), 1);
    ...
    fn closed_mode_takes_the_closed_fields_and_keeps_the_inferred_profit() {
        let out = blend(vec![inferred("a", 9.0, 70.0, 69.0, 12.0)], vec![closed("a", 30.0, 66.0)], PriceSourceMode::Closed, 10, ProfitBasis::Spread, now());
        let e = &out[0];
    ...
    fn closed_mode_falls_back_per_key_and_keeps_closed_only_keys() {
        let out = blend(vec![inferred("only_inferred", 9.0, 70.0, 69.0, 12.0)], vec![closed("only_closed", 30.0, 66.0)], PriceSourceMode::Closed, 10, ProfitBasis::Spread, now());
        assert_eq!(out.len(), 2);
    ...
    #[test]
    fn the_range_basis_replaces_profit_only_in_closed_mode_and_only_with_a_range() {
        let run = |mode, basis, range_profit| {
            blend(vec![inferred("a", 9.0, 70.0, 69.0, 12.0)], vec![ClosedStats { range_profit, ..closed("a", 30.0, 66.0) }], mode, 10, basis, now()).remove(0)
        };
        assert_eq!(run(PriceSourceMode::Closed, ProfitBasis::Spread, Some(30.0)).stats.profit, Some(12.0), "the default keeps the inferred spread");
        assert_eq!(run(PriceSourceMode::Closed, ProfitBasis::Range, Some(30.0)).stats.profit, Some(30.0), "the daily range is upstream's profit");
        assert_eq!(run(PriceSourceMode::Closed, ProfitBasis::Range, None).stats.profit, Some(12.0), "no range, no change");
        let identity = run(PriceSourceMode::Inferred, ProfitBasis::Range, Some(30.0));
        assert_eq!(identity.stats.profit, Some(12.0), "the basis never reaches inferred mode");
        assert_eq!((identity.stats.moving_avg, identity.closed), (Some(70.0), false));
    ...
    fn the_guard_lowers_closed_avg_only_past_the_percentage_with_enough_trades_and_when_enabled() {
        let run = |volume: f64, avg_48h: f64, pct: i64| blend(vec![inferred("a", volume, 99.0, avg_48h, 5.0)], vec![closed("a", 30.0, 100.0)], PriceSourceMode::Closed, pct, ProfitBasis::Spread, now()).remove(0);

...
impl Client<Authenticated> {
    ...
    */
    pub async fn refresh(&self) -> Result<String, ApiError> {
        ...
        // quantframe-server patch: auctions (v1 /profile/{name}/auctions) are not fetched.
        match user.tier {
            Some(tier) => {
                if tier.is_premium() {
        ...
        // quantframe-server patch: chats (v1 /im/chats) are not fetched.
        Ok("Successfully refreshed user data".to_string())
    }
}

#[cfg(test)]
mod quantframe_server_patch_tests {
    /// Guards the vendored patch: signing in must not trigger v1 auction or chat requests.
    #[test]
    fn refresh_does_not_fetch_v1_auctions_or_chats() {
        let source = include_str!("client.rs");
        let start = source.find("pub async fn refresh(").expect("refresh() exists");
        let body = &source[start..source[start..].find("\n    }\n").map(|i| start + i).unwrap()];
        assert!(body.contains("my_orders()"), "refresh() should still load orders");
        for forbidden in ["my_auctions()", "get_chats()"] {
            assert!(!body.contains(forbidden), "refresh() must not call {forbidden}");
        }
    }
}

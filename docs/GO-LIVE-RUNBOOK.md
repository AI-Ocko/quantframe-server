...
- [ ] **Settings → Live Scraper → Item → WTB**: **Trading tax cap** and **Price shift threshold** hold values you chose on purpose. `-1` turns each off. The tax cap applies in both price modes (150 000 excludes exactly the items taxed at a million credits or more); the shift threshold applies in Closed trades mode, and only exactly `-1` disables it.

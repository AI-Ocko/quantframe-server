...
- **P12 — Amendments from the final whole-branch review (2026-09-20).** These override P2–P4 and P8 where they differ.
  ...
  - **Window anchored on the fetch.** An item fetched before the current cutoff cannot contain `today − 1`, so its window is the seven days ending `today − 2`; an item fetched at or after the cutoff uses the seven days ending `today − 1`. `load_fresh` applies this per item (by calling `aggregate` with the matching end day); `aggregate` itself is unchanged. Without this, every item not yet refetched today would read a six-day window and `volume` at 6⁄7 of the truth, a daily sawtooth in the candidate set and a false "systematic factor" in P11's calibration.

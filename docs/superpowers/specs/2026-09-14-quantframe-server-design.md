...
- **warframe.market sign-in** uses `POST /v1/auth/signin`. The only other permitted v1 call is the one-off statistics backfill (§24). Everything else uses v2. Only the token is kept (valid ~60 days), encrypted with AES-256-GCM. The password is never stored.
